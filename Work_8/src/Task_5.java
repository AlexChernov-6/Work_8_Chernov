...
public class Task_5 {
    ...
    static String mapLink = "https://recyclemap.ru/";
    ...
    public static void main(String[] args) {
        ...
        */
        ...
        if (ChatCommand.MAP.name().equals(commandValue)) {
            System.out.println(mapLink);
        }
        else if (ChatCommand.RECYCLABILITY.name().equals(commandValue)) {
            System.out.println("Введите код переработки: ");
            isRecycled(scanner.nextInt());
        }
        else if (ChatCommand.BONUS.name().equals(commandValue)) {
            System.out.println("Введите количество вторсырья, кг: ");
            double answerWeight = scanner.nextDouble();
            double scores = answerWeight < 10 ?  answerWeight * 10 : answerWeight * 15;
            System.out.printf("Количество бонусных баллов: %.2f.", scores);
        }
        else
            System.out.println("Указанной команды не существует");
    }

    // добавьте модификаторы в заголовок метода
    protected static void isRecycled(int code) {
